# AI Career Agent Coach - AWS Deployment Script
...
Write-Host "SUCCESS: Build complete!`n" -ForegroundColor Green

# Step 2: Check if bucket exists
Write-Host "Step 2: Checking S3 bucket..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    ...
    $bucketPolicy | ConvertTo-Json -Depth 10 | Out-File -FilePath bucket-policy.json -Encoding UTF8
    ...
    Write-Host "SUCCESS: Bucket created and configured!`n" -ForegroundColor Green
}
else {
    Write-Host "SUCCESS: Bucket already exists!`n" -ForegroundColor Green
}
...
Write-Host "SUCCESS: Files uploaded!`n" -ForegroundColor Green
...
if ($DistributionId) {
    ...
    Write-Host "SUCCESS: Cache invalidation started (ID: $($invalidation.Invalidation.Id))`n" -ForegroundColor Green
}
else {
    Write-Host "SKIPPED: Step 4 (No CloudFront distribution ID provided)" -ForegroundColor Yellow
